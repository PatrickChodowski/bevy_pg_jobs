...
// impl DynamicTyped for dyn PGTask {
//     fn reflect_type_info(&self) -> &'static TypeInfo {todo!()}
// }


// impl DynamicTyped for dyn PGTask {
//     fn reflect_type_info() -> &'static TypeInfo {todo!()}
// }
...
//         if let Some(task) = reflect.as_any().downcast_ref::<Box<dyn PGTask>>() {
//             Some(task.clone()) // Ensure PGTask is Clone if needed
//         } else {
//             None
//         }
//     }
...
// struct BoxedTask(Box<dyn PGTask + 'static>);



// impl Task {
    // pub fn idtn(id: u32, next: u32, task: Task) -> Self {
    //     Task{id, next: Some(next), task}
    // }
